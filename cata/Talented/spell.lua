local function handle_ranks(...)
	...
	if c == 45 then -- '-' Skip bad talent (MAGE / cata)
		row, column = nil, -1
		c = 0
	elseif c == 42 then -- '*' Alternate talent (PETs)
		row, column = nil, -1
		pos = pos + 1
...
function Talented:UncompressSpellData(class)
	assert(class, "UncompressSpellData called without a parameter")
	local data = self.spelldata[class:upper()]
	assert(data, ("There is no spell data found for class `%s'"):format(class))
